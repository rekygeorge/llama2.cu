...
// Optimized GPU attention kernels with better memory access patterns
__global__ void compute_attention_scores_optimized_kernel(
    const float* Q,           // [n_heads, head_size]
    const float* K_cache,     // [seq_len, n_kv_heads, head_size]
    ...
    for (int i = tid; i < head_size; i += blockDim.x) {
        float q_val = Q[head_idx * head_size + i];
        float k_val = K_cache[pos_idx * n_kv_heads * head_size + kv_head_idx * head_size + i];
        sum += q_val * k_val;
    }
    
    // Warp-level reduction (more efficient than shared memory for this case)
    for (int offset = warpSize / 2; offset > 0; offset /= 2) {
        sum += __shfl_down_sync(0xffffffff, sum, offset);
    }
    
    if (tid % warpSize == 0) {
        atomicAdd(&scores[head_idx * seq_len + pos_idx], sum);
    }
    
    // Ensure all warps have contributed, then normalize
    __syncthreads();
    if (tid == 0) {
        scores[head_idx * seq_len + pos_idx] /= sqrtf((float)head_size);
    }
}

__global__ void apply_softmax_optimized_kernel(float* scores, int n_heads, int seq_len) {
    int head_idx = blockIdx.x;
    if (head_idx >= n_heads) return;
    
    float* head_scores = scores + head_idx * seq_len;
    
    // Find max with single pass
    float max_val = -INFINITY;
    for (int i = 0; i < seq_len; i++) {
        max_val = fmaxf(max_val, head_scores[i]);
    }
    
    // Compute exp and sum in single pass
    float sum = 0.0f;
    for (int i = 0; i < seq_len; i++) {
        float exp_val = expf(head_scores[i] - max_val);
        head_scores[i] = exp_val;
        sum += exp_val;
    }
    ...
    for (int i = 0; i < seq_len; i++) {
        head_scores[i] *= inv_sum;
    }
}

__global__ void compute_attention_output_optimized_kernel(
    const float* V_cache,     // [seq_len, n_kv_heads, head_size]
    const float* scores,      // [n_heads, seq_len]
    float* output,            // [n_heads, head_size]
    int seq_len,
    int n_heads,
    int n_kv_heads,
    int head_size
    ...
    if (head_idx >= n_heads || dim_idx >= head_size) return;
    
    int kv_head_idx = head_idx / (n_heads / n_kv_heads);
    const float* head_scores = scores + head_idx * seq_len;
    
    // Compute weighted sum with better memory access pattern
    float result = 0.0f;
    for (int pos = 0; pos < seq_len; pos++) {
        float v_val = V_cache[pos * n_kv_heads * head_size + kv_head_idx * head_size + dim_idx];
        result += head_scores[pos] * v_val;
    }
    
    output[head_idx * head_size + dim_idx] = result;
}
...
// High-performance matrix multiplication kernel with tiling
__global__ void matmul_kernel_tiled(float* xout, const float* x, const float* w, int n, int d) {
    const int TILE_SIZE = 32;
    
    __shared__ float shared_x[TILE_SIZE];
    __shared__ float shared_w[TILE_SIZE];
    ...
    for (int tile = 0; tile < (n + TILE_SIZE - 1) / TILE_SIZE; tile++) {
        ...
        // Compute partial dot product
        int tile_size = min(TILE_SIZE, n - tile * TILE_SIZE);
        for (int k = 0; k < tile_size; k++) {
            result += shared_w[k] * shared_x[k];
        }
...
void cuda_flash_attention(
    ...
    float* d_scores;
    CUDA_CHECK(cudaMalloc(&d_scores, n_heads * seq_len * sizeof(float)));
    CUDA_CHECK(cudaMemset(d_scores, 0, n_heads * seq_len * sizeof(float)));
    
    // Step 1: Compute attention scores with optimized kernel
    dim3 blockSize1(256);
    dim3 gridSize1(n_heads, seq_len);
    compute_attention_scores_optimized_kernel<<<gridSize1, blockSize1>>>(
        d_q, d_key_cache, d_scores, seq_len, n_heads, n_kv_heads, head_size
    );
    CUDA_CHECK(cudaGetLastError());
    
    // Step 2: Apply softmax with optimized kernel
    dim3 blockSize2(1);
    dim3 gridSize2(n_heads);
    apply_softmax_optimized_kernel<<<gridSize2, blockSize2>>>(d_scores, n_heads, seq_len);
    CUDA_CHECK(cudaGetLastError());
    
    // Step 3: Compute weighted sum with optimized kernel
    dim3 blockSize3(256);
    dim3 gridSize3(n_heads, (head_size + blockSize3.x - 1) / blockSize3.x);
    compute_attention_output_optimized_kernel<<<gridSize3, blockSize3>>>(
        d_val_cache, d_scores, d_output, seq_len, n_heads, n_kv_heads, head_size
    );
    CUDA_CHECK(cudaGetLastError());
...
// Global variable to track CUDA availability
...
#if USE_CUDA
        if (g_cuda_available) {
            // Full GPU acceleration with optimized attention
...
#if USE_CUDA
    if (cuda_available) {
        printf("Flash Attention: FULLY OPTIMIZED GPU ACCELERATION\n");
    } else {
