...
float* forward_gpu(Transformer* transformer, int token, int pos) {
    ...
    for (int l = 0; l < p->n_layers; l++) {
        ...
        if (pos < 10 && l == 0){
            printf("\n\nLayer %llu, Position %d: RMSNorm applied to input\n", l, pos);
            // Debugging: Print the first 10 elements of d_rms_att_weight and d_x
            float *host_x = (float*)malloc(dim * sizeof(float));
            float sum_x = 0.0f;
            float *host_rms_att_weight = (float*)malloc( p->n_layers * p->dim * sizeof(float));
            float sum_rms_att_weight = 0.0f;
            CUDA_CHECK(cudaMemcpy(host_rms_att_weight, w->d_rms_att_weight + l * dim, p->n_layers * p->dim * sizeof(float), cudaMemcpyDeviceToHost));
            CUDA_CHECK(cudaMemcpy(host_x, s->d_x, dim * sizeof(float), cudaMemcpyDeviceToHost));

            printf("\n d_rms_att_weight : [ ");
            for(int i = 0; i < 10; i++) {
                printf("%.4f, ", host_rms_att_weight[i]);
            }

            // Calculate sum of d_rms_att_weight
            for (int i = 0; i < p->n_layers * p->dim ; i++){
                sum_rms_att_weight += host_rms_att_weight[i];
            }
            ...
            // Sum of values in d_x
            for (int i = 0; i < dim; i++){
                sum_x += host_x[i];
            }
            printf("Sum of d_x: %.4f\n", sum_x);
            printf("Sum of d_rms_att_weight: %.4f \n\n", sum_rms_att_weight);

            free(host_rms_att_weight);
            free(host_x);
        }
        ...
        if (pos < 10 && l == 0){
            printf("Layer %d, Position %d: RMSNorm applied to input\n", l, pos);
            
        //     // Copy GPU data to CPU for printing
        //     float* host_xb = (float*)malloc(10 * sizeof(float));
        //     CUDA_CHECK(cudaMemcpy(host_xb, s->d_xb, 10 * sizeof(float), cudaMemcpyDeviceToHost));
        ...
        //     free(host_xb);
            ...
            // Sum all the values in d_xb (copy to CPU first)
            float* host_xb = (float*)malloc(dim * sizeof(float));
            CUDA_CHECK(cudaMemcpy(host_xb, s->d_xb, dim * sizeof(float), cudaMemcpyDeviceToHost));
            
            float sum_xb = 0.0f;
            for (int i = 0; i < dim; i++){
                sum_xb += host_xb[i];
            }
            printf("Sum of d_xb: %.6f\n", sum_xb);
        ...
        // Apply RoPE on GPU
        int total_elements = 1 * 1 * p->n_heads * head_size;  // batch=1, seq=1 for single token
        dim3 blockDim(256);
