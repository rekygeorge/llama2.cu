...
#if USE_CUDA
        if (g_cuda_available) {
            // GPU-accelerated matrix operations, CPU attention
            ...
            // Copy QKV back to host for attention computation
            ...
            // Cache management and attention (CPU - known working implementation)
            ...
            // Copy attention output back to GPU and continue with GPU operations
...
#if USE_CUDA
    if (cuda_available) {
        printf("Flash Attention: HYBRID (GPU matrix ops, CPU attention)\n");
    } else {
