...
void memory_map_weights(TransformerWeights *w, Config* p, float* ptr, int shared_weights) {
    ...
    size_t rms_att_size = n_layers * p->dim;
    printf("\n2. Attention RMSNorm Weights:\n");
...
float* forward_gpu(Transformer* transformer, int token, int pos) {
    ...
    for (int l = 0; l < p->n_layers; l++) {
        ...
        cuda_flash_attention_fixed(
            s->d_xb,                    // output: [1, 1, n_heads, head_size] 
            d_query_reshaped,           // query:  [1, 1, n_heads, head_size]
            d_key_cache_layer,          // key:    [1, pos+1, n_kv_heads, head_size]
            d_value_cache_layer,        // value:  [1, pos+1, n_kv_heads, head_size] 
            1,                          // batch_size = 1
        ...
        cublasSgemv(cublas_handle, CUBLAS_OP_T, dim, dim, &alpha,
                   layer_wo, dim, s->d_xb, 1, &beta, s->d_xb2, 1);
