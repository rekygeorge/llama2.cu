...
float* forward_gpu(Transformer* transformer, int token, int pos) {
    ...
    for (int l = 0; l < p->n_layers; l++) {
        ...
        // **MULTI-HEAD FLASH ATTENTION IMPLEMENTATION**
        // printf("Applying Multi-Head Flash Attention on GPU for layer %d, position %d\n", l, pos);
        
        // Prepare attention inputs - reshape Q, K, V for multi-head processing
        float* d_query_reshaped = s->d_q;    // [n_heads * head_size] -> treat as [n_heads, head_size]
        ...
        // Process each attention head separately using FlashAttention
        // for (int head = 0; head < p->n_heads; head++) {
        //     // Calculate KV head index (for Grouped Query Attention)
        //     int kv_head = head % p->n_kv_heads;
            
        //     // Offset pointers for this head
        //     float* q_head = d_query_reshaped + head * head_size;           // [head_size]
        //     float* k_cache_head = d_key_cache_layer + kv_head * head_size; // [seq_len, head_size] strided
        //     float* v_cache_head = d_value_cache_layer + kv_head * head_size; // [seq_len, head_size] strided
        //     float* o_head = s->d_xb + head * head_size;                    // [head_size] output location
            
        //     // Calculate optimal block sizes based on available SRAM
        //     const int M = 16 * 1024;  // 48KB SRAM (adjust based on your GPU)
        //     const int Bc = M / (4 * head_size);
        //     const int Br = min(Bc, head_size);
            
        //     // Ensure block sizes are reasonable
        //     // int actual_Bc = min(Bc, pos + 1);  // Don't exceed sequence length
        //     // int actual_Br = min(Br, 32);       // Reasonable row block size

        //     int actual_Bc = min(min(Bc, 16), pos + 1);  // Cap at 16
        //     int actual_Br = min(Br, 16);                // Cap at 16
            
        //     // Launch FlashAttention kernel for this head
        //     dim3 grid_fa(1);  // Single block for current implementation
        //     dim3 block_fa(256);
        ...
        //         1 * head_size +            // Qi (single query)
        //         1 * actual_Bc +            // Sij (single row)
        //         1 * actual_Bc +            // Pij (single row)
        //         1 * head_size +            // Oi_local (single row)
        //         6                          // Statistics (li, mi, etc.)
        //     );
            
        //     // Launch single-query FlashAttention kernel
        //     single_query_flashattention_kernel<<<grid_fa, block_fa, smem_size>>>(
        //         q_head,                    // Query for this head [head_size]
        //         k_cache_head,              // Key cache [seq_len, head_size] with stride
        //         v_cache_head,              // Value cache [seq_len, head_size] with stride  
        //         o_head,                    // Output for this head [head_size]
        //         pos + 1,                   // Sequence length (causal: up to current position)
        //         head_size,                 // Head dimension
        //         actual_Bc,                 // Column block size
        //         p->n_kv_heads * head_size, // Stride for K,V cache (to next position)
        ...
        const int M = 48 * 1024;  // 32KB for RTX 2070 Super
