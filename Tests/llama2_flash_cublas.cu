...
void profile_7b_bottlenecks(Transformer* transformer, int token, int pos) {
    ...
    printf("\n=== Assignment_05 7B MODEL BOTTLENECK ANALYSIS (Layer 0, pos=%d) ===\n", pos);
    ...
    // cuda_matmul_optimized(s->d_q, s->d_xb, w->d_wq + l*dim*dim, dim, dim);
    // cuda_matmul_optimized(s->d_k, s->d_xb, w->d_wk + l*dim*kv_dim, dim, kv_dim);
    // cuda_matmul_optimized(s->d_v, s->d_xb, w->d_wv + l*dim*kv_dim, dim, kv_dim);

    // Compute Q = Wq * xb
    // cublas_matmul(s->d_q, s->d_xb, w->d_wq + l*dim*dim, dim, dim);
    
    // // Compute K = Wk * xb  
    // cublas_matmul(s->d_k, s->d_xb, w->d_wk + l*dim*kv_dim, dim, kv_dim);
    
    // // Compute V = Wv * xb
    // cublas_matmul(s->d_v, s->d_xb, w->d_wv + l*dim*kv_dim, dim, kv_dim);
    ...
    //     s->d_xb, layer_w1, layer_w3, s->d_hb, s->d_hb2,
    //     dim, hidden_dim, alpha, beta
    // );
    ...
    // float* layer_w2 = w->d_w2 + l * hidden_dim * dim;
    cublasSgemv(cublas_handle, CUBLAS_OP_T, hidden_dim, dim, &alpha,
                layer_w2, hidden_dim, s->d_hb, 1, &beta, s->d_xb, 1);
    ...
    printf("ESTIMATED 32-LAYER TIME: %.1f ms = %.1f tok/s\n", 
           total_time * 32, 1000.0f / (total_time * 32));
...
void configure_shared_memory_limits() {
    ...
    // err = cudaFuncSetAttribute(cuda_matmul_optimized, cudaFuncAttributeMaxDynamicSharedMemorySize, max_shared_mem);
    // if (err != cudaSuccess) printf("Warning: Failed to set shared memory for cuda_matmul_optimized\n");
...
void cuda_matmul(float* d_xout, float* d_x, float* d_w, int n, int d) {
    int blockSize = 64;  // started with 256,
...
float* forward_gpu(Transformer* transformer, int token, int pos) {
    ...
    if (pos == 10) {  // Profile at position 10
        profile_7b_bottlenecks(transformer, token, pos);
        return transformer->state.logits;  // Exit early after profiling
    }
    ...
    for (int l = 0; l < p->n_layers; l++) {
        ...
        // printf("\n=== Layer %d Profiling (pos=%d) ===\n", l, pos);
    
        // ----------------------------------------------------------------------------------

        // 1. Profile RMSNorm (attention)
        ...
        // printf("RMSNorm (attention): %.3f ms\n", elapsed_time);

        // if (err != cudaSuccess) {
        //     fprintf(stderr,"Failed to record or synchronize event: %s\n", cudaGetErrorString(err));
        //     return NULL;  // Return NULL on error
        // }else{
        //     fprintf(stderr,"RMSNorm (attention): %.3f ms\n", elapsed_time);
        // }
        ...
        // if (err != cudaSuccess) {
        //     printf("Failed to record or synchronize event: %s\n", cudaGetErrorString(err));
        //     return NULL;  // Return NULL on error
        // }else{
        //     printf("QKV Projections: %.3f ms\n", elapsed_time);
        // }
        ...
        // cudaEventSynchronize(stop);
        // cudaEventElapsedTime(&elapsed_time, start, stop);
        // printf("Output Proj + Residual: %.3f ms\n", elapsed_time);
        ...
        // cudaEventSynchronize(stop);
        // cudaEventElapsedTime(&elapsed_time, start, stop);
        // printf("FFN W1+W3 Projections: %.3f ms\n", elapsed_time);
        ...
        // cudaEventSynchronize(stop);
        // cudaEventElapsedTime(&elapsed_time, start, stop);
        // printf("FFN W2 Projection: %.3f ms\n", elapsed_time);
        ...
        // cudaEventSynchronize(stop);
        // cudaEventElapsedTime(&elapsed_time, start, stop);
        // printf("Fused SwiGLU and FFN W2 projection: %.3f ms\n", elapsed_time);


        // -------------------------------------------------------------------------------
        
        // 11. Profile Final Residual
        ...
        // cudaEventRecord(stop);
        // cudaEventSynchronize(stop);
        // cudaEventElapsedTime(&elapsed_time11, start, stop);
        // printf("Final Residual: %.3f ms\n", elapsed_time);
    ...
    cublasSgemv(cublas_handle, CUBLAS_OP_T, dim, p->vocab_size, &alpha,
               w->d_wcls, dim, s->d_x, 1, &beta, s->d_logits, 1);
