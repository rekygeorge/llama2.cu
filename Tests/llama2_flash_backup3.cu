...
// Simple attention computation on GPU - mirrors CPU implementation
void cuda_simple_attention(
    const float* d_q,        // Query on device
    const float* d_key_cache,// Key cache on device
    const float* d_val_cache,// Value cache on device
    float* d_output,         // Output on device
    float* h_q,              // Host query buffer
    float* h_key_cache,      // Host key cache buffer
    float* h_val_cache,      // Host value cache buffer  
    float* h_output,         // Host output buffer
    float* h_att,            // Host attention buffer
    ...
    // Copy data from device to host for attention computation
    CUDA_CHECK(cudaMemcpy(h_q, d_q, n_heads * head_size * sizeof(float), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(h_key_cache, d_key_cache, seq_len * kv_dim * sizeof(float), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(h_val_cache, d_val_cache, seq_len * kv_dim * sizeof(float), cudaMemcpyDeviceToHost));
    
    // Multi-head attention (CPU implementation)
    for (int h = 0; h < n_heads; h++) {
        float* q = h_q + h * head_size;
        float* att = h_att + h * seq_len;
        
        // Compute attention scores
        for (int t = 0; t < seq_len; t++) {
            float* k = h_key_cache + t * kv_dim + (h / kv_mul) * head_size;
            float score = 0.0f;
        ...
        float* xb = h_output + h * head_size;
        memset(xb, 0, head_size * sizeof(float));
        for (int t = 0; t < seq_len; t++) {
            float* v = h_val_cache + t * kv_dim + (h / kv_mul) * head_size;
            float a = att[t];
            for (int i = 0; i < head_size; i++) {
                xb[i] += a * v[i];
            }
    ...
    CUDA_CHECK(cudaMemcpy(d_output, h_output, n_heads * head_size * sizeof(float), cudaMemcpyHostToDevice));
}
...
void cuda_flash_attention(
    ...
    // For now, just use a simple implementation that mirrors CPU
    // We'll optimize this later once we get correct results
    printf("Warning: Using CPU-based attention computation\n");
}
...
float* forward(Transformer* transformer, int token, int pos) {
    ...
    for(unsigned long long l = 0; l < p->n_layers; l++) {
        ...
        if (g_cuda_available) {
            ...
            // Optimized: Only copy what we need for attention
            ...
            // Cache management and attention (CPU - optimized)
            ...
            // Process all heads efficiently
            #pragma omp parallel for if(p->n_heads > 8)
            for (int h = 0; h < p->n_heads; h++) {
                float* q = s->q + h * head_size;
                float* att = s->att + h * p->seq_len;
                
                // Compute scores with better cache usage
                ...
                for (int t = 0; t <= pos; t++) {
                    ...
                    for (int i = 0; i < head_size; i += 4) {
                        score += q[i] * k[i];
                        if (i + 1 < head_size) score += q[i+1] * k[i+1];
                        if (i + 2 < head_size) score += q[i+2] * k[i+2];
                        if (i + 3 < head_size) score += q[i+3] * k[i+3];
                    }
                ...
                for (int t = 0; t <= pos; t++) {
                    ...
                    for (int i = 0; i < head_size; i += 4) {
                        xb[i] += a * v[i];
                        if (i + 1 < head_size) xb[i+1] += a * v[i+1];
                        if (i + 2 < head_size) xb[i+2] += a * v[i+2];
                        if (i + 3 < head_size) xb[i+3] += a * v[i+3];
                    }
                }
            }

            // Copy attention output back to GPU efficiently
            CUDA_CHECK(cudaMemcpyAsync(s->d_xb, s->xb, dim * sizeof(float), cudaMemcpyHostToDevice, 0));
            
            // Continue with GPU operations
...
#if USE_CUDA
    if (cuda_available) {
        printf("Flash Attention: OPTIMIZED HYBRID (GPU matrix ops, optimized CPU attention)\n");
    } else {
