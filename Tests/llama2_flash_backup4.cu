...
// Simple but efficient GPU attention kernels
__global__ void compute_attention_scores_kernel(
    ...
    if (tid == 0) {
        scores[head_idx * seq_len + pos_idx] = shared_sum[0] / sqrtf((float)head_size);
    }
}

__global__ void apply_softmax_kernel(float* scores, int n_heads, int seq_len) {
    int head_idx = blockIdx.x;
    ...
    for (int i = 0; i < seq_len; i++) {
        head_scores[i] = expf(head_scores[i] - max_val);
        sum += head_scores[i];
    }
...
void cuda_flash_attention(
    ...
    compute_attention_scores_kernel<<<gridSize1, blockSize1>>>(
    ...
    apply_softmax_kernel<<<gridSize2, blockSize2>>>(d_scores, n_heads, seq_len);
    CUDA_CHECK(cudaGetLastError());
    
    // Step 3: Compute weighted sum
    dim3 blockSize3(256);
    dim3 gridSize3(n_heads, (head_size + blockSize3.x - 1) / blockSize3.x);
    compute_attention_output_kernel<<<gridSize3, blockSize3>>>(
...
#if USE_CUDA
    if (cuda_available) {
        printf("Flash Attention: FULLY GPU ACCELERATED\n");
    } else {
