...
def compile_and_run_cuda_small(
    ...
    return _compile_and_run_cuda_impl(model, prompt, steps, temperature, topp, seed, use_boto3, save_output, output_file)
...
def _compile_and_run_cuda_impl(
    ...
    required_files = ["llama2_flash.cu"]
    ...
    executable_path = "/app/llama2_flash"
    ...
    execution_strategies = [
        ...
        },
        {
            ...
            "cmd": run_cmd,
            "env_vars": {"CUDA_LAUNCH_BLOCKING": "1", "CUDA_DEVICE_ORDER": "PCI_BUS_ID"},
            "timeout": 180
        },
        {
            "name": "Fallback GPU execution",
            "cmd": run_cmd,
            "env_vars": {"CUDA_VISIBLE_DEVICES": "0", "CUDA_CACHE_DISABLE": "1"},
            "timeout": 180
        }
    ]
            ...
            if return_code == 0:
                ...
                if return_code == -11:
                    ...
                    if strategy_num == len(execution_strategies) - 1:
                        ...
                        _run_detailed_segfault_analysis(log_and_capture, current_cmd, final_cuda_arch if 'final_cuda_arch' in locals() else cuda_arch)
                    else:
...
def _run_detailed_segfault_analysis(log_and_capture, run_cmd, cuda_arch):
    ...
    if core_files:
        ...
        try:
            ...
            gdb_cmd = ["gdb", "--batch", "--ex", "bt", f"/app/llama2_flash", f"/app/{core_file}"]
    ...
    try:
        objdump_result = subprocess.run(["objdump", "-s", "-j", ".nv_fatbin", "/app/llama2_flash"], 
...
def list_profiling_files():
    ...
    if os.path.exists(cache_dir):
        profiling_files = []
        for item in os.listdir(cache_dir):
            if item.endswith('.txt') and ('profiling' in item.lower() or 'output' in item.lower()):
                item_path = os.path.join(cache_dir, item)
                if os.path.isfile(item_path):
                    size = os.path.getsize(item_path)
...
def main(
    ...
    if success:
        ...
        if save_output:
            ...
            print(f"💡 You can retrieve the file using: modal volume get huggingface-cache {output_file}")
    else:
