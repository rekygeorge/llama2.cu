 ...
 *
 * Build (Windows):
 *   nvcc run_fp16.cu win.c -o run_fp16_cuda -lcublas -lm -O2
 * Build (WSL/Linux):
 *   nvcc run_fp16.cu -o run_fp16_cuda -lcublas -lm -O2
 * Usage (same CLI as run.c):
 *   ./run_fp16_cuda model.bin -n 256 -i "Once upon a time" [--verify]
 */
