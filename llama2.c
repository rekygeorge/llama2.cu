...
// Missing includes and CUDA kernels for Flash Attention implementation
...
// Add missing fields to TransformerWeights structure
typedef struct {
...
__global__ void flash_attention_kernel(
    float* output,           // [batch_size, seq_len, n_heads, head_dim]
    float* query,            // [batch_size, seq_len, n_heads, head_dim]  
    float* key,              // [batch_size, seq_len, n_kv_heads, head_dim]
    float* value,            // [batch_size, seq_len, n_kv_heads, head_dim]
    int batch_size,
    int seq_len,
    int n_heads,
    int n_kv_heads,
    ...
    // Calculate which KV head to use (for grouped-query attention)
    const int kv_head_idx = head_idx * n_kv_heads / n_heads;
    
    // Shared memory for tile-based computation
    extern __shared__ float shared_mem[];
    float* shared_k = shared_mem;
    float* shared_v = shared_mem + TILE_SIZE * head_dim;
    float* shared_scores = shared_mem + 2 * TILE_SIZE * head_dim;
    
    // Initialize output accumulators
    float output_acc[VECTORIZE_WIDTH] = {0.0f};
    float max_score = -INFINITY;
    float sum_exp = 0.0f;
    
    // Query vector for this position
    const int q_offset = ((batch_idx * seq_len + seq_idx) * n_heads + head_idx) * head_dim;
    
    // Process keys and values in tiles
    for (int tile_start = 0; tile_start <= seq_idx; tile_start += TILE_SIZE) {
        const int tile_end = min(tile_start + TILE_SIZE, seq_idx + 1);
        ...
        for (int i = tid; i < tile_size * head_dim; i += blockDim.x) {
            const int tile_pos = i / head_dim;
            const int dim_idx = i % head_dim;
            const int global_pos = tile_start + tile_pos;
            
            if (global_pos <= seq_idx) {
                const int k_offset = ((batch_idx * seq_len + global_pos) * n_kv_heads + kv_head_idx) * head_dim;
                const int v_offset = ((batch_idx * seq_len + global_pos) * n_kv_heads + kv_head_idx) * head_dim;
                
                shared_k[tile_pos * head_dim + dim_idx] = key[k_offset + dim_idx];
                shared_v[tile_pos * head_dim + dim_idx] = value[v_offset + dim_idx];
            }
        }
        __syncthreads();
        ...
        for (int pos = 0; pos < tile_size; pos++) {
            ...
            for (int d = 0; d < head_dim; d++) {
                score += query[q_offset + d] * shared_k[pos * head_dim + d];
            }
        ...
        float tile_sum = 0.0f;
        for (int pos = 0; pos < tile_size; pos++) {
            float exp_score = expf(shared_scores[pos] - max_score);
            shared_scores[pos] = exp_score;
            tile_sum += exp_score;
            
            // Accumulate weighted values
            for (int d = tid; d < head_dim; d += blockDim.x) {
                output_acc[d % VECTORIZE_WIDTH] += exp_score * shared_v[pos * head_dim + d];
            }
        }
        
        sum_exp = sum_exp * expf(max_score - max_score) + tile_sum;
        __syncthreads();
    }
    
    // Write final output with normalization
    const int out_offset = ((batch_idx * seq_len + seq_idx) * n_heads + head_idx) * head_dim;
    for (int d = tid; d < head_dim; d += blockDim.x) {
        output[out_offset + d] = output_acc[d % VECTORIZE_WIDTH] / sum_exp;
    }
}
...
void read_checkpoint(char* checkpoint, Config* config, TransformerWeights* weights,
    ...
    *file_size = ftell(file); // get the file size, in bytes
    ...
    #ifdef _WIN32
        *fd = win_open(checkpoint, O_RDONLY); // Use your Windows open replacement
        if (*fd == -1) { fprintf(stderr, "open failed!\n"); exit(EXIT_FAILURE); }
...
long time_in_ms() {
#ifdef _WIN32
    return win_time_in_ms(); // Implement this in win.c
#else
    struct timespec time;
    clock_gettime(CLOCK_REALTIME, &time);
    return time.tv_sec * 1000 + time.tv_nsec / 1000000;
...
void flash_attention_cpu(
    ...
    float* query,            // [seq_len, n_heads, head_dim]  
    float* key,              // [seq_len, n_kv_heads, head_dim]
    float* value,            // [seq_len, n_kv_heads, head_dim]
    int seq_len,
    int n_heads,
    int n_kv_heads, 
    int head_dim,
    ...
    for (int h = 0; h < n_heads; h++) {
        ...
        // Initialize output
        for (int d = 0; d < head_dim; d++) {
            output[h * head_dim + d] = 0.0f;
        }
        
        // Compute attention scores and apply softmax
        float scores[seq_len];
        for (int t = 0; t <= pos; t++) { // Causal masking
            float* k = key + t * n_kv_heads * head_dim + kv_head * head_dim;
        ...
        for (int t = 0; t <= pos; t++) {
            float* v = value + t * n_kv_heads * head_dim + kv_head * head_dim;
            float weight = scores[t] / sum_exp;
            
            for (int d = 0; d < head_dim; d++) {
                output[h * head_dim + d] += weight * v[d];
            }
        }
    }
}
...
float* forward(Transformer* transformer, int token, int pos) {
    ...
    for (int l = 0; l < p->n_layers; l++) {
        ...
        // Flash Attention
        if (g_cuda_available) {
            // Use CUDA Flash Attention (simplified - would need full implementation)
            printf("Using Flash Attention GPU implementation at layer %d, pos %d\n", l, pos);
            
            // For now, fall back to CPU implementation
            flash_attention_cpu(s->xb, s->q, s->key_cache + loff, s->value_cache + loff,
