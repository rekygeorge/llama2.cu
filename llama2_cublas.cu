...
static char g_profile_csv_path[512] = "cublas_profile_metrics.csv";
#ifdef DUMP_LOGITS
...
void profile_7b_bottlenecks(Transformer* transformer, int token, int pos) {
    ...
    // at the next cudaDeviceSynchronize() call (line 762 in cuda_matmul_2d_tiled).
    ...
    // Catch any stale sticky error before the launch so that a launch failure
    // is reported at this stage rather than blamed on the next kernel.
    cudaGetLastError();
    ...
    cuda_matmul_2d_tiled(s->d_xb2, s->d_xb, w->d_wo + l*dim*dim, dim, dim);
...
float* forward_gpu(Transformer* transformer, int token, int pos) {
    ...
    for (int l = 0; l < p->n_layers; l++) {
        ...
        // Cublas implementation for Q, K, V projections
        // Q = xb * Wq^T
        // const float alpha = 1.0f, beta = 0.0f;
        cublasSgemv(cublas_handle, CUBLAS_OP_T, dim, dim, &alpha, 
                   layer_wq, dim, s->d_xb, 1, &beta, s->d_q, 1);
        ...
        if (profile_layer0) {
            cudaEventRecord(prof_stop);
            cudaEventSynchronize(prof_stop);
            cudaEventElapsedTime(&stage3_ms, prof_start, prof_stop);
        }
        ...
        if (profile_layer0) {
            cudaEventRecord(prof_stop);
            cudaEventSynchronize(prof_stop);
            cudaEventElapsedTime(&stage4_ms, prof_start, prof_stop);
        }
        ...
        // Cublas implementation for W1 and W3 projections
        cublasSgemv(cublas_handle, CUBLAS_OP_T, dim, hidden_dim, &alpha,
                   layer_w1, dim, s->d_xb, 1, &beta, s->d_hb, 1);
        cublasSgemv(cublas_handle, CUBLAS_OP_T, dim, hidden_dim, &alpha, 
                   layer_w3, dim, s->d_xb, 1, &beta, s->d_hb2, 1);
...
void error_usage() {
    ...
    fprintf(stderr, "  -R <string> profiling CSV output path (default: cublas_profile_metrics.csv)\n");
    exit(EXIT_FAILURE);
}
